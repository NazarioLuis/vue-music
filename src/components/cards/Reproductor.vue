<template>
  <div id="player" v-if="cancion.title">
    ...
    <div class="info row">
      ...
      <div class="currently-playing">
        <h2 class="song-name">{{cancion.title}}</h2>
        <h3 class="artist-name">{{cancion.artist.name}}</h3>
      </div>
    </div>
  </div>
...
<script>
  export default {
      ...
      albumStyles() {
        return {
          "background": `linear-gradient(rgba(54, 79, 60, 0.25),
           rgba(73, 101, 77, 0.55)), url(${this.cancion.album.cover_big })`,
        };
      },
      fillStyles() {
    ...
    mounted() {
      this.$audio.src = this.cancion.preview;
      var _this = this;
